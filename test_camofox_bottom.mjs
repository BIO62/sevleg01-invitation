...
async function run() {
  ...
  const bottomImg = await req(`/tabs/${tabId}/screenshot?userId=preview-user-4`, 'GET');
  fs.writeFileSync(path.join(ARTIFACT_DIR, 'clone_bottom_footer.png'), bottomImg);
  console.log('Saved clone_bottom_footer.png');

  await req(`/tabs/${tabId}?userId=preview-user-4`, 'DELETE');
}
